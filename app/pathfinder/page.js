...
const careerDetails = {
  Builder: {
    ...
    list: ["Civil Engineering Tech", "Aircraft Mechanic", "Power Engineer", "Construction Manager", "Renewable Energy Tech"]
  },
  Healer: {
    ...
    <main className="min-h-screen bg-white text-[#1a1a1a] antialiased flex flex-col">
      ...
      <nav className="w-full border-b border-gray-100 py-4 px-8 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-[100]">
        <Link href="/campus" className="flex items-center gap-2 group">
          <div className="w-8 h-8 bg-[#b22222] rounded-full flex items-center justify-center text-white font-bold text-xs transition-transform group-hover:scale-110">A</div>
        ...
        {step === -1 ? (
          ...
          <div className="flex flex-col animate-in fade-in duration-700">
            ...
            <div className="max-w-2xl mx-auto px-6 py-16 text-center md:text-left">
              <span className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-4 block">Major Discovery Tool</span>
              ...
              <p className="text-lg text-gray-600 mb-10 leading-relaxed">
                Everyone learns differently. Answer a few questions about your habits and preferences to find the best industry and learning format for the way you live.
              </p>
              <button 
                onClick={() => setStep(0)}
              ...
              <p className="mt-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Takes about 3 minutes</p>
            </div>
          </div>
            ...
            <div className="grid gap-4">
              {questions[step].options.map((opt, i) => (
                ...
                >
                  ...
                  <span className="text-2xl opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-2 text-blue-600">→</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto px-8 py-20 text-center animate-in zoom-in-95 duration-700">
            <div className={`w-20 h-20 ${result.color} rounded-3xl flex items-center justify-center text-4xl mx-auto mb-6 shadow-sm border`}>
              {result.icon}
            </div>
            ...
            <h2 className="text-6xl font-black mb-4 text-slate-900 tracking-tighter italic">
              The {winnerKey}
            </h2>
            
            <p className="text-gray-500 text-lg mb-12 max-w-xl mx-auto">
              Your profile aligns perfectly with high-demand roles in Alberta. Based on your {winnerKey} traits, explore these careers:
            </p>

            {/* CAREER LIST GRID */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-12 max-w-2xl mx-auto">
              {result.list.map((job, i) => (
                <div key={i} className="bg-slate-50 border border-slate-100 p-5 rounded-2xl flex items-center gap-4 hover:border-blue-200 hover:bg-white transition-all group">
